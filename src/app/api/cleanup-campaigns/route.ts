...
export async function POST() {
  try {
    ...
    if (process.env.DB_ENGINE === 'local') {
      ...
      // For local database, we can't easily get count, so just clear everything
      await db.close()
    ...
    try {
      ...
      result.steps.push(`Supabase failed: ${supabaseError instanceof Error ? supabaseError.message : 'Unknown error'}`)
